...
SERIAL_PORT = "COM6"
BAUD_RATE = 115200
HEARTBEAT_DEADLINE_SECONDS = 90  # seconds
...
def process_serial_line(raw_line):
    if not raw_line:
        return
...
def check_dead_nodes():
    ...
    for node, last_message_time in list(last_heard.items()):
        if now - last_message_time > HEARTBEAT_DEADLINE_SECONDS:
...
def start_gateway():
    ...
    try:
        ...
        while True:
            ...
            process_serial_line(raw_line)

    except serial.SerialException as e:
        affected_nodes = [
            node for node, port in node_registry.items() if port == SERIAL_PORT
        ]
        error_message = build_message(
            type="system_log",
            node="gateway_script",
